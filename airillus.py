...
import numpy as np    #numpy to create array of zeros for the paint screen
import cv2			  # imported OpenCV
from collections import deque   #dequeue is used for making color boxes 
...
# to draw on canvas
bpoints = [deque(maxlen = 1024)]		#blue box
gpoints = [deque(maxlen = 1024)]  		#green box
rpoints = [deque(maxlen = 1024)]		#red box
ypoints = [deque(maxlen = 1024)]		#yellow box
...
paintWindow = np.zeros((471, 636, 3)) + 255   #np.zeros will give black screen +255 will change it to white

cv2.namedWindow('ILLUSTRATOR', cv2.WINDOW_AUTOSIZE)
...
while True:
	...
	cv2.putText(frame, "CLEAR ALL", (49, 33),
				...
				(255, 255, 255), 2, cv2.LINE_AA)   #For better look, lineType = cv.LINE_AA is recommended.
